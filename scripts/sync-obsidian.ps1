...
WriteLog "Starting Obsidian Memory Vault synchronization..."

# Verify paths
if (-not (Test-Path $workspacePath)) {
    WriteLog "Error: Workspace path $workspacePath does not exist."
    exit 1
}

if (-not (Test-Path $vaultPath)) {
    WriteLog "Error: Obsidian vault path $vaultPath does not exist."
    exit 1
}

try {
    # 1. Fetch Git Repository Information
    WriteLog "Querying Git status for F:\Final..."
    ...
    WriteLog "Git Info: Branch=[$gitBranch], LastCommit=[$gitLastCommit], ModifiedFiles=[$modifiedCount]"

    # 2. Retrieve Workspace Structure Statistics (Optimized - Skip Ignored Folders)
    WriteLog "Calculating workspace stats (skipping node_modules, .git, etc.)..."
    ...
    WriteLog "Workspace stats: TotalFiles=[$totalFiles]"
    ...
    WriteLog "Writing daily note to $dailyNotePath..."
    ...
    if (Test-Path $ledgerPath) {
        WriteLog "Appending sync log to Daily Activity Ledger.md..."
    ...
    if (Test-Path $enginePath) {
        WriteLog "Updating local workspace references in Sierra Estates Memory Engine.md..."
        ...
        if ($engineContent.Contains($oldPathPattern)) {
            ...
            WriteLog "Successfully updated absolute file links in Sierra Estates Memory Engine.md."
        }
    }

    WriteLog "Obsidian Memory Vault synchronization completed successfully."

} catch {
    $err = $_.Exception.Message
    WriteLog "Critical Error during Obsidian sync: $err"
    exit 1
}
