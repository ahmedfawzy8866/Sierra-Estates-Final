...
import { cos_sim, buf_to_vec, vec_to_buf } from "../utils/index"; // eslint-disable-line @typescript-eslint/no-unused-vars
...
import {
    q,
    vector_store,
    get_async, // eslint-disable-line @typescript-eslint/no-unused-vars
    all_async, // eslint-disable-line @typescript-eslint/no-unused-vars
    run_async, // eslint-disable-line @typescript-eslint/no-unused-vars
    transaction,
    log_maint_op,
} from "../core/db";
...
export function calc_mean_vec(
    ...
    secs: string[], // eslint-disable-line @typescript-eslint/no-unused-vars
): number[] {
...
export async function create_single_waypoint(
    ...
    const thresh = 0.75; // eslint-disable-line @typescript-eslint/no-unused-vars
    const mems = user_id
...
import {
    ...
    embedMultiSector,
    cosineSimilarity,
    bufferToVector, // eslint-disable-line @typescript-eslint/no-unused-vars
    vectorToBuffer,
    EmbeddingResult,
} from "./embed";
import { chunk_text } from "../utils/chunking";
import { j } from "../utils";
import { keyword_filter_memories, extract_keywords } from "../utils/keyword"; // eslint-disable-line @typescript-eslint/no-unused-vars
import {
    calculateCrossSectorResonanceScore,
    applyRetrievalTraceReinforcementToMemory,
    propagateAssociativeReinforcementToLinkedNodes,
    ALPHA_LEARNING_RATE_FOR_RECALL_REINFORCEMENT, // eslint-disable-line @typescript-eslint/no-unused-vars
    BETA_LEARNING_RATE_FOR_EMOTIONAL_FREQUENCY, // eslint-disable-line @typescript-eslint/no-unused-vars
} from "../ops/dynamics";
...
const seg_cache = new Map<number, any /* eslint-disable-line @typescript-eslint/no-explicit-any */[]>();
const coact_buf: Array<[string, string]> = [];
const TTL = 60000;
const VEC_CACHE_MAX = 1000; // eslint-disable-line @typescript-eslint/no-unused-vars
let active_queries = 0;

const get_segment = async (seg: number): Promise<any /* eslint-disable-line @typescript-eslint/no-explicit-any */[]> => { // eslint-disable-line @typescript-eslint/no-unused-vars
...
const get_sal = async (id: string, def_sal: number): Promise<number> => { // eslint-disable-line @typescript-eslint/no-unused-vars
...
export async function hsg_query(
    ...
    try {
        ...
        const is_temporal = has_temporal_markers(qt); // eslint-disable-line @typescript-eslint/no-unused-vars
        ...
        for (const mid of Array.from(ids)) {
            ...
            for (const [sec, rr] of Object.entries(sr)) {
                ...
                if (mat && mat.similarity > bs) {
                    bs = mat.similarity;
                    bsec = sec; // eslint-disable-line @typescript-eslint/no-unused-vars
                }
            }
        ...
        for (const r of top) {
            ...
            if (r.path.length > 1) {
                ...
                const lns = wps.map((wp: any /* eslint-disable-line @typescript-eslint/no-explicit-any */) => ({
                    target_id: wp.dst_id,
                    weight: wp.weight,
                }));
                const pru =
