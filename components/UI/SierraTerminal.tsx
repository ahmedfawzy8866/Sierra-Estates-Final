...
interface SierraTerminalProps {
  isInline?: boolean;
}

export default function SierraTerminal({ isInline = false }: SierraTerminalProps) {
  ...
  if (isInline) {
    return (
      <div 
        className="w-full h-[32rem] bg-[#101B27] border border-gold/20 shadow-2xl flex flex-col overflow-hidden"
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-gold/10 flex items-center justify-between bg-black/20 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-white font-serif font-bold text-lg leading-none m-0">Sierra AI</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6" dir="auto">
          {messages.map((msg) => (
            ...
            >
              ...
              <div className={`px-4 py-3 text-sm ${
                msg.sender === 'sierra'
                  ? 'bg-[#1e2d4a] text-slate-200 rounded-2xl rounded-tl-sm border border-white/5'
                  : 'bg-gold text-[#101B27] font-medium rounded-2xl rounded-tr-sm'
              }`}>
                {msg.text}
                {msg.isVIP && (
          ...
          {isLoading && (
            <div className="flex gap-3 flex-row">
              ...
              <div className="px-4 py-3 bg-[#1e2d4a] rounded-2xl rounded-tl-sm flex items-center gap-1">
        ...
        <div className="p-4 border-t border-gold/10 bg-black/20">
          <div className="relative flex items-center">
            ...
            <button 
              ...
              className="absolute end-2 w-8 h-8 rounded-full bg-gold text-[#101B27] flex items-center justify-center hover:bg-white disabled:opacity-50 transition-colors"
            >
      ...
      <AnimatePresence>
        {!isOpen ? (
          <motion.button
            ...
            className="w-16 h-16 rounded-full bg-[#182535] text-gold flex items-center justify-center shadow-2xl border-2 border-gold/30 hover:border-gold transition-colors"
          >
          ...
          <motion.div
            ...
            className="w-80 sm:w-96 h-[32rem] bg-[#101B27] border border-gold/20 shadow-2xl shadow-navy/50 flex flex-col overflow-hidden"
            style={{ borderRadius: '24px' }}
          >
            {/* Header */}
            <div className="px-6 py-4 border-b border-gold/10 flex items-center justify-between bg-black/20 backdrop-blur-md">
