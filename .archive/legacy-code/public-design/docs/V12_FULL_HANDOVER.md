...
- **Path structure:** The active workspace is `h:\sierra estates SaaS Program Locally V2\my-app`. (Drives C: and F: contain deprecated / legacy backups and should be ignored).
...
3. **Run Commands:**
   ```bash
   cd "h:\sierra estates SaaS Program Locally V2\my-app"
   npm install
   npm run dev
   ```
