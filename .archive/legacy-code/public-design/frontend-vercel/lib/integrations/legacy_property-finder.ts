// sierra-estates/lib/integrations/property-finder.ts
...
export async function pushListingToPF(listing: SBRListing): Promise<PFSyncResult> {
  try {
    ...
    const payload = {
      ...
      // sierra estates custom fields (PF supports extra metadata)
      custom_fields: {
...
export async function getPFListingAnalytics(pfListingId: string) {
  ...
  // Normalize PF analytics into sierra estates format
  return {
    pfListingId,
