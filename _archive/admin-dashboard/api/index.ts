...
async function getHermesAIResponse(conversationId: string, userMessage: string): Promise<string> {
  ...
  if (GEMINI_API_KEY) {
    try {
      ...
      // Define tools for Hermes multitasking operations
      const savePropertyListingTool = {
        name: 'savePropertyListing',
        description: 'Save or update a real estate property listing in the inventory.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            title: { type: Type.STRING, description: 'Title of the property' },
            type: { type: Type.STRING, description: 'Type of property e.g. Apartment, Villa, Townhouse' },
            cmp: { type: Type.STRING, description: 'Compound name e.g. Madinaty, Uptown Cairo' },
            price: { type: Type.NUMBER, description: 'Price in EGP' },
            beds: { type: Type.NUMBER, description: 'Number of bedrooms' },
            baths: { type: Type.NUMBER, description: 'Number of bathrooms' },
            area: { type: Type.NUMBER, description: 'Area in square meters' }
          },
          required: ['title', 'type', 'cmp', 'price']
        }
      };

      const createViewingRequestTool = {
        name: 'createViewingRequest',
        description: 'Create a property viewing or tour request for a client.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            clientPhone: { type: Type.STRING, description: 'Client phone number in international format' },
            propertyRef: { type: Type.STRING, description: 'Reference ID or code of the property e.g. SE001' },
            preferredDate: { type: Type.STRING, description: 'Date and time of the tour e.g. next Saturday 2pm' }
          },
          required: ['clientPhone', 'propertyRef', 'preferredDate']
        }
      };
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          tools: [{
            functionDeclarations: [savePropertyListingTool, createViewingRequestTool]
          }]
        }
      });

      // Handle function calling (Tools) execution
      if (response && response.functionCalls && response.functionCalls.length > 0) {
        const parts: any[] = response.functionCalls.map(call => ({
          functionCall: {
            name: call.name,
            args: call.args
          }
        ...
        for (const call of response.functionCalls) {
          ...
          if (call.name === 'savePropertyListing') {
            ...
            try {
              if (db) {
                ...
                result = { success: true, id: docRef.id, message: 'Property successfully added to listings inventory.' };
                console.log(`[Hermes Tool] Saved property to Firestore with ID: ${docRef.id}`);
              } else {
                result = { success: false, error: 'Database not initialized' };
              }
            } catch (err: any) {
              result = { success: false, error: err.message };
            }
          } else if (call.name === 'createViewingRequest') {
            const args = call.args as any;
            try {
              if (db) {
                const docRef = await db.collection('viewing_requests').add({
                  ...args,
                  status: 'pending',
                  createdAt: admin.firestore.FieldValue.serverTimestamp()
                });
                result = { success: true, id: docRef.id, message: 'Viewing request successfully scheduled.' };
                console.log(`[Hermes Tool] Created viewing request with ID: ${docRef.id}`);
              } else {
                result = { success: false, error: 'Database not initialized' };
              }
            } catch (err: any) {
        ...
        // Call Gemini again with the tool output in history
        ...
        if (secondResponse && secondResponse.text) {
          const reply = secondResponse.text.trim();
