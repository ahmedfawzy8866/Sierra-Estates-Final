...
import { InvestmentStakeholder, StakeholderDraft, StakeholderFilters, PipelinePhase, PHASE_SEQUENCE, CHANNEL_METADATA, INITIAL_STAKEHOLDER_STATE } from './types';
...
export const matchesStakeholderFilters = (stakeholder: InvestmentStakeholder, filters: StakeholderFilters) => {
...
export function useKanbanBoard() {
  ...
  useEffect(() => {
    ...
    const unsubscribePartners = onSnapshot(pq, (snapshot) => {
      setPartners(snapshot.docs.map(d => ({ id: d.id, name: d.data().name })));
    });
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const allStakeholders: InvestmentStakeholder[] = snapshot.docs.map(d => {
        const data = d.data();
